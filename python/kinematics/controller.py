...
class Controller:
    ...
    def get_position_jacobian(self):
        ...
        return Jp # np.round(np.vstack((Jo, Jp)), 2)
    ...
    def get_orientation_jacobian(self):
        ...
        return Jo #np.round(np.vstack((Jo, Jp)), 2)
    ...
    def inverse_kinematics(self, pose_desired, arg = 'euler'):
        tolerance = 1e-6
        K = 3
        e = np.zeros(6)
        dp = np.zeros(6)
        q = self.get_joint_variable()
        pose_current = self.forward_kinematics(arg)
        ...
        if np.linalg.norm(e) > tolerance:
            ...
            dq = np.linalg.solve(J.T.dot(J), J.T.dot(dp)) #dq = np.dot(np.linalg.inv(J.T.dot(J)), J.T.dot(e))
        ...
        print("pose new    ", self.forward_kinematics(arg))
        return q_new
