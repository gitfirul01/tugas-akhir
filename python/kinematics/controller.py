...
class Controller:
    ...
    def plot_workspace(self, ax):
        ## linear space joint limits and convert to radian
        theta1 = np.linspace(self.joint_limits[0][0], self.joint_limits[0][1], 20)
        theta2 = np.linspace(self.joint_limits[1][0], self.joint_limits[1][1], 20)
        theta3 = np.linspace(self.joint_limits[2][0], self.joint_limits[2][1], 20)
        
        # theta1, theta2, theta3 = np.meshgrid(theta1, theta2, theta3)
        # x = 150*np.cos(np.deg2rad(theta1))*np.cos(np.deg2rad(theta2)) - 150*np.cos(np.deg2rad(theta1))*np.cos(np.deg2rad(theta2))*np.sin(np.deg2rad(theta3)) - 150*np.cos(np.deg2rad(theta1))*np.cos(np.deg2rad(theta3))*np.sin(np.deg2rad(theta2))
        # y = 150*np.cos(np.deg2rad(theta2))*np.sin(np.deg2rad(theta1)) - 150*np.cos(np.deg2rad(theta2))*np.sin(np.deg2rad(theta1))*np.sin(np.deg2rad(theta3)) - 150*np.cos(np.deg2rad(theta3))*np.sin(np.deg2rad(theta1))*np.sin(np.deg2rad(theta2))
        # z = 150*np.sin(np.deg2rad(theta2)) + 150*np.cos(np.deg2rad(theta2))*np.cos(np.deg2rad(theta3)) - 150*np.sin(np.deg2rad(theta2))*np.sin(np.deg2rad(theta3)) + 45
        ...
        for i in range(len(theta1)):
            for j in range(len(theta2)):
                for k in range(len(theta3)):
                    T = mdh2tf(0, 0, self.d1, np.rad2deg(np.deg2rad(theta1)[i])) \
                        @ mdh2tf(self.alpha1, self.a1, self.d2, np.rad2deg(np.deg2rad(theta2)[j])) \
                        @ mdh2tf(self.alpha2, self.a2, self.d3, np.rad2deg(np.deg2rad(theta3)[k])) \
    ...
    def forward_kinematics(self, q = None, unit = 'degree', rep = 'euler'):
        ...
        if q != None:
            if unit == 'degree':
                self.theta1 = normalize_joint_range(q[0] % 360)
                self.theta2 = normalize_joint_range(q[1] % 360)
                self.theta3 = normalize_joint_range(q[2] % 360)
                self.theta4 = normalize_joint_range(q[3] % 360)
                self.theta5 = normalize_joint_range(q[4] % 360)
                self.theta6 = normalize_joint_range(q[5] % 360)
            elif unit == 'radian':
                self.theta1 = normalize_joint_range(np.rad2deg(q[0]) % 360)
                self.theta2 = normalize_joint_range(np.rad2deg(q[1]) % 360)
                self.theta3 = normalize_joint_range(np.rad2deg(q[2]) % 360)
                self.theta4 = normalize_joint_range(np.rad2deg(q[3]) % 360)
                self.theta5 = normalize_joint_range(np.rad2deg(q[4]) % 360)
                self.theta6 = normalize_joint_range(np.rad2deg(q[5]) % 360)
